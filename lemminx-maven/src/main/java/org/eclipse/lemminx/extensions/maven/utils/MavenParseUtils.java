...
public class MavenParseUtils {
	...
	public static Parent parseParent(DOMNode node) {
		...
		if (node.isElement()) {
			Parent res = parseParentInternal((DOMElement) node);
			if (res != null) {
				return res;
			}
			return parseParentInternal(node.getParentElement());
		}
		return parseParent(node.getParentElement());
	}
	...
	private static boolean isEmpty(Parent res) {
		return res.getGroupId() == null && res.getArtifactId() == null && res.getVersion() == null
				&& res.getRelativePath() == null;
	}
}
